...
from hbaselines.goal_conditioned_com_2.td3 import GoalConditionedPolicy
from hbaselines.goal_conditioned_mb_3.td3 import GoalConditionedMBPolicy
...
class OffPolicyRLAlgorithm(object):
    ...
    def __init__(self,
        ...
        elif policy == GoalConditionedPolicy or GoalConditionedMBPolicy:
        ...
        elif hasattr(self.env, "max_actions"):
            self.horizon = self.env.max_actions
    ...
    def _policy(self,
        ...
        action, message = self.policy_tf.get_action(
            obs, context, message=previous_message,
        ...
        return action.flatten(), q_value, message
    ...
    def _store_transition(self,
        ...
        self.policy_tf.store_transition(
            obs0, context0, action, message0, messgae1, reward, obs1, context1, terminal1,
    ...
    def learn(self,
        ...
        with self.sess.as_default(), self.graph.as_default():
            ...
            message_variables = tf.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES)
            message_variables_to_restore = [v for v in message_variables if
                                            v.name.split('/')[1] == 'communication']
    ...
    def _collect_samples(self,
        ...
        for _ in range(run_steps or self.nb_rollout_steps):
            ...
            # replay buffer.
            action, (q1_value, q2_value), message = self._policy(
                self.obs, context, self.message,
                apply_noise=True,
                random_actions=random_actions,
                compute_q=True)
    ...
    def _train(self):
        ...
        for t_train in range(self.nb_train_steps):
            if (self.policy == GoalConditionedPolicy) or (self.policy == GoalConditionedMBPolicy):
            ...
            loss_lists = self.policy_tf.update(
                update_actor=update, **kwargs)

            if len(loss_lists) == 2:
                critic_loss, actor_loss = loss_lists
                cg_loss, w_dynamic_loss = 0, 0
            else:
                critic_loss, actor_loss, cg_loss = loss_lists
            ...
            self.epoch_cg_losses.append(cg_loss)
            # self.epoch_dynamic_losses.append(w_dynamic_loss)
    ...
    def _evaluate(self, total_timesteps, env):
        ...
        if isinstance(self.policy_tf, GoalConditionedPolicy) or isinstance(self.policy_tf, GoalConditionedMBPolicy):
        ...
        for i in range(self.nb_eval_episodes):
            ...
            while True:
                ...
                eval_action, _, message = self._policy(
                    eval_obs, context, previous_message=eval_message,
                ...
                # of the policy.
                self._store_transition(eval_obs, context0, eval_action, eval_message, message, eval_r,
                                       obs, context1, False, is_final_step=eval_episode_step >= self.horizon - 1,
                                       evaluate=True, )

                # Update the previous step observation.
                eval_obs = obs.copy()
                eval_message = message.copy()
    ...
    def _log_training(self, file_path, start_time):
        ...
        combined_stats = {
            ...
            'train/loss_cg': np.mean(self.epoch_cg_losses),
            # 'train/loss_dynamic': np.mean(self.epoch_dynamic_losses),
