...
class ManagerFeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self,
        ...
        """
        super(ManagerFeedForwardPolicy, self).__init__(
            sess=sess,
            ob_space=ob_space,
            ac_space=ac_space,
            co_space=co_space,
        ...
        with tf.compat.v1.variable_scope("input", reuse=False):
            ...
            self.message_ph = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None,) + (8,),
                name='message')
            self.message1_ph = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None,) + (8,),
                name='message1')
        ...
        with tf.compat.v1.variable_scope("model", reuse=False):
            self.actor_tf, self.z_mean, self.z_std = self.make_actor(obs_ph, self.message_ph)
            self.critic_tf = [
                self.make_critic(obs_ph, self.message_ph, self.action_ph,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]
            self.critic_with_actor_tf = [
                self.make_critic(obs_ph, self.message_ph, self.actor_tf, reuse=True,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]
        ...
        with tf.compat.v1.variable_scope("target", reuse=False):
            ...
            actor_target, _, _ = self.make_actor(obs1_ph, self.message1_ph)
            ...
            critic_target = [
                self.make_critic(obs1_ph, self.message1_ph, noisy_actor_target,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]
    ...
    def _setup_actor_optimizer(self, scope):
        ...
        kl_div_loss = 1 + self.z_std - tf.square(self.z_mean) - tf.exp(self.z_std)
        kl_div_loss = -0.5 * tf.reduce_sum(kl_div_loss, 1)
        self.actor_loss += 0.05 * tf.reduce_mean(kl_div_loss)
    ...
    def make_actor(self, obs, message, reuse=False, scope="pi"):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            pi_h = tf.concat([obs, message], axis=-1)
            ...
            for i, layer_size in enumerate(self.layers[:1]):
                pi_h = self._layer(
                    pi_h, layer_size, 'fc_{}'.format(i),
                    act_fun=self.act_fun,
                    layer_norm=self.layer_norm
                )

            z_mean = self._layer(
                pi_h, self.ac_space.shape[0], 'z_mean',
                act_fun=None,
                kernel_initializer=tf.orthogonal_initializer()
            )
            z_std = self._layer(
                pi_h, self.ac_space.shape[0], 'z_std',
                act_fun=None,
                kernel_initializer=tf.orthogonal_initializer()
            )
            ...
            eps = tf.random_normal(tf.shape(z_std), dtype=tf.float32, mean=0., stddev=1.0,
                                   name='epsilon')
            pi_h = z_mean + tf.exp(z_std / 2) * eps

            # pi_h = tf.concat([obs, pi_h], axis=-1)

            for i, layer_size in enumerate(self.layers):
                pi_h = self._layer(
                    pi_h, layer_size, 'after_ib_fc_{}'.format(i),
                    act_fun=self.act_fun,
                    layer_norm=self.layer_norm
                )
        ...
        return policy, z_mean, z_std

    def make_critic(self, obs, message, action, reuse=False, scope="qf"):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            ...
            qf_h = tf.concat([obs, message, action], axis=-1)
    ...
    def update_from_batch(self,
        ...
        critic_loss, *_vals = self.sess.run(step_ops, feed_dict={
            ...
            self.message_ph: message0,
            self.message1_ph: message1,
        })

        # Extract the actor loss.
        actor_loss = _vals[5] if update_actor else 0
    ...
    def get_action(self, obs, context, message, apply_noise, random_actions):
        ...
        if random_actions:
            ...
            action = self.sess.run(self.actor_tf, {self.obs_ph: obs, self.message_ph: message})
    ...
    def get_intrinsic_reward(self, obs, context, message):
        if context[0] is not None:
            obs = self._get_obs(obs, context, axis=1)
        else:
            obs = obs
        z_mean, z_std = self.sess.run([self.z_mean, self.z_std], {self.obs_ph: obs, self.message_ph: message})
        kl_div_loss = 1 + z_std - np.power(z_mean, 2) - np.exp(z_std)
        kl_div_loss = np.clip(kl_div_loss, -1., 1.)
        kl_div_loss = -0.5 * np.mean(kl_div_loss)
    ...
    def get_td_map_from_batch(self, obs0, actions, rewards, obs1, terminals1, message0, message1):
