...
class WorkerFeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self,
        ...
        """
        super(WorkerFeedForwardPolicy, self).__init__(
            sess=sess,
            ob_space=ob_space,
            ac_space=ac_space,
            co_space=co_space,
        ...
        with tf.compat.v1.variable_scope("input", reuse=False):
            ...
            self.action_com_ph = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None,) + ac_space.shape,
                name='actions')

            self.obs_m_ph = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None, ReportTime) + ob_dim,
                name='obs_m')

            self.action_m_ph = tf.compat.v1.placeholder(
                tf.float32,
                shape=(None, ReportTime) + ac_space.shape,
                name='action_m')

        if image:
            obs_ph = self.obs_ph * tf.constant([0.] * 2 + [1.] * (self.obs_ph.shape[-1] - 2), dtype=tf.float32)
            obs1_ph = self.obs1_ph * tf.constant([0.] * 2 + [1.] * (self.obs_ph.shape[-1] - 2), dtype=tf.float32)
        else:
            obs_ph = self.obs_ph
            obs1_ph = self.obs1_ph
        ...
        with tf.compat.v1.variable_scope("model", reuse=False):
            self.actor_tf = self.make_actor(obs_ph)
            self.critic_tf = [
                self.make_critic(obs_ph, self.action_ph,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]
            self.critic_with_actor_tf = [
                self.make_critic(obs_ph, self.actor_tf, reuse=True,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]

        with tf.compat.v1.variable_scope("communication", reuse=False):
            self.message_tf = self.make_message(self.obs_m_ph, self.action_m_ph)

            self.pre_action = self.make_condition(obs_ph, self.message_tf)

        with tf.compat.v1.variable_scope("target", reuse=False):
            # create the target actor policy
            actor_target = self.make_actor(obs1_ph)
            ...
            critic_target = [
                self.make_critic(obs1_ph, noisy_actor_target,
                                 scope="qf_{}".format(i))
                for i in range(2)
            ]
        ...
        with tf.compat.v1.variable_scope("Optimizer", reuse=False):
            ...
            self._setup_message_loss(scope)
    ...
    def _setup_message_loss(self, scope):
        scope_name = 'model/communication/'
        if scope is not None:
            scope_name = scope + '/' + scope_name

        # compute the actor loss
        # kl_div_loss = 1 + self.message_std - tf.square(self.message_mean) - tf.exp(self.message_std)
        ...
        self.message_loss = loss_fn(self.pre_action, self.action_ph)# + 0.05 * self.kl_div_loss

        optimizer = tf.compat.v1.train.AdamOptimizer(self.actor_lr)
        self.message_optimizer = optimizer.minimize(self.message_loss)

    def make_condition(self, obs, message, reuse=False, scope="condition"):
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            # concatenate the observations and actions
            action_h = tf.concat([obs, message], axis=-1)

            # create the hidden layers
            for i, layer_size in enumerate(self.layers[1:]):
                action_h = self._layer(
                    action_h, layer_size, 'fc{}'.format(i),
                    act_fun=self.act_fun,
                    layer_norm=self.layer_norm
                )

            action = self._layer(
                action_h, self.ac_space.shape[0], 'output',
                act_fun=tf.nn.tanh,
            ...
            action = ac_means + ac_magnitudes * action
    ...
    def make_message(self, obs, action, reuse=False, scope="communication"):
        """Create a critic tensor.
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            # concatenate the observations and actions
            com_h = tf.concat([obs, action], axis=-1)


            for i, layer_size in enumerate(self.layers[:1]):
                com_h = self._layer(
                    com_h,  layer_size/2, 'fc{}'.format(i),
                    act_fun=self.act_fun,
                    layer_norm=self.layer_norm
                )

            com_h = tf.reshape(com_h, [-1, com_h.shape[1]*com_h.shape[2]])

            for i, layer_size in enumerate(self.layers[1:]):
                com_h = self._layer(
                    com_h,  layer_size/2, 'fc{}'.format(i+1),
                    act_fun=self.act_fun,
                    layer_norm=self.layer_norm
                )
            ...
            #
            # outputs, states = tf.nn.dynamic_rnn(cell, com_h, dtype=tf.float32)
    ...
    def get_message(self, obs_m, action_m):
        """See parent class."""
        # Add the contextual observation, if applicable.
        message = self.sess.run(self.message_tf, {self.obs_m_ph: obs_m, self.action_m_ph: action_m})
