...
from hbaselines.fcnet.td3_worker_message_3 import WorkerFeedForwardPolicy, ReportTime
from hbaselines.fcnet.td3_manager_com import ManagerFeedForwardPolicy
from hbaselines.utils.tf_util import get_trainable_vars
from hbaselines.goal_conditioned_com_2.replay_buffer import HierReplayBuffer
...
class GoalConditionedPolicy(ActorCriticPolicy):
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope("Manager"):
            self.manager = ManagerFeedForwardPolicy(
                sess=sess,
                ob_space=ob_space,
        ...
        self.worker_obs_m = np.zeros([ReportTime, ob_space.shape[0] + manager_ac_space.shape[0]])
        self.worker_act_m = np.zeros([ReportTime, ac_space.shape[0]])
        ...
        with tf.compat.v1.variable_scope("Worker"):
            self.worker = WorkerFeedForwardPolicy(
                sess,
                ob_space=ob_space,
                ac_space=ac_space,
    ...
    def update(self, update_actor=True, **kwargs):
        ...
        **Note**; The target update soft updates for both the manager and the
        ...
        worker_obs1, worker_act, worker_rew, worker_done, message0, message1, worker_obs_m, worker_act_m = \
        ...
        if self.connected_gradients:
            ...
            w_critic_loss, w_actor_loss = self._connected_gradients_update(
                obs0=worker_obs0,
                actions=worker_act,
                rewards=worker_rew,
                obs1=worker_obs1,
                ...
                manager_obs0=meta_obs0,
                manager_actions=meta_act,
                manager_obs1=meta_obs1,
            )
        else:
            # w_critic_loss, w_actor_loss = 0, 0
        ...
        message_loss = self.message_update(obs0=worker_obs0[1:],
                                           actions=worker_act[1:],
                                           rewards=worker_rew[1:],
                                           obs1=worker_obs1[1:],
                                           terminals1=worker_done[1:],
                                           worker_obs_m=worker_obs_m[:-1],
                                           worker_act_m=worker_act_m[:-1]
                                           )

        return (m_critic_loss, w_critic_loss), (m_actor_loss, w_actor_loss)

    def get_action(self, obs, context, message, apply_noise, random_actions):
        """See parent class."""
        if self._update_meta:
            ...
            self.meta_action = self.manager.get_action(
                obs, context, message, apply_noise, random_actions)
        else:
        ...
        add_obs_for_m = np.array(self._get_obs(obs, self.meta_action, 1), copy=False)
        add_act_for_m = np.array(worker_action, copy=False)

        self.worker_obs_m = np.vstack([add_obs_for_m, self.worker_obs_m[:-1]])
        self.worker_act_m = np.vstack([add_act_for_m, self.worker_act_m[:-1]])

        message = self.worker.get_message(self.worker_obs_m.reshape(1,ReportTime,-1), self.worker_act_m.reshape(1,ReportTime,-1))

        return worker_action, message

    def value(self, obs, context, action):
        """See parent class."""
        return 0, 0  # FIXME

    def store_transition(self, obs0, context0, action, message0, message1, reward, obs1, context1,
        ...
        intrinsic_reward = self.manager.get_intrinsic_reward([obs0], [context0], message0)
        self.meta_reward += reward + 0.5 * intrinsic_reward

        # print(obs0[0:2], context0, message1[0], intrinsic_reward)
        with open('/home/cil-c42/Projects/h-baselines/experiments/ib_250000.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow([obs0[0:2], context0, message1[0], reward, intrinsic_reward])
        ...
        if len(self._observations) == self.meta_period or done:
            ...
            if not evaluate:
                self.replay_buffer.add(
                    ...
                    message_t=(self.prev_message, message1)
                )
        ...
        if done:
            self.worker_obs_m = np.zeros([ReportTime, obs0.shape[0] + self.meta_action.shape[-1]])
            self.worker_act_m = np.zeros([ReportTime, action.shape[0]])
    ...
    def get_td_map(self):
        ...
        td_map.update(self.manager.get_td_map_from_batch(
            meta_obs0, meta_act, meta_rew, meta_obs1, meta_done, message0, message1))
    ...
    def _setup_connected_gradients(self):
        ...
        obs = tf.concat([self.worker.obs1_ph[:, :obs_shape], self.manager.obs_ph[:, obs_shape:]], axis=-1)
        ...
        kl_div_loss = 1 + self.worker.message_std - tf.square(self.worker.message_mean) - tf.exp(
            self.worker.message_std)
        ...
        self.cg_optimizer = optimizer.minimize(
            0.1 * self.worker_message_loss + self.cg_weights * self.cg_loss + self.worker.actor_loss,
            var_list=get_trainable_vars("Worker/communication/") + get_trainable_vars("Worker/model/pi"),
        )
        # m_optimizer = tf.compat.v1.train.AdamOptimizer(self.worker.actor_lr)
        # self.message_optimizer = m_optimizer.minimize(
        #     self.cg_weights * self.worker_message_loss,
    ...
    def _connected_gradients_update(self,
        ...
        step_ops = [self.worker.critic_loss,
                    self.worker.critic_optimizer[0],
                    self.worker.critic_optimizer[1]]

        feed_dict = {
            self.worker.obs_ph: obs0,
            self.worker.action_ph: actions,
            self.worker.rew_ph: rewards,
            self.worker.obs1_ph: obs1,
            self.worker.terminals1: terminals1
        }

        if update_actor:
            # Actor updates and target soft update operation.
            step_ops += [self.worker.actor_loss,
                         self.cg_optimizer,  # This is what's replaced.
                         # self.message_optimizer,
                         # self.worker.actor_optimizer,
                         self.worker.target_soft_updates,
                         self.worker_message_loss,
                         self.cg_loss]

            feed_dict.update({
                self.manager.obs_ph: manager_obs0,
                self.manager.action_ph: manager_actions,
                self.manager.obs1_ph: manager_obs1,
            })
    ...
    def message_update(self,
        ...
        # Update operations for the critic networks.
        step_ops = [self.worker.message_loss,
                    self.worker.message_optimizer]
        ...
        feed_dict = {
            ...
            self.worker.obs1_ph: obs1,
            self.worker.terminals1: terminals1,
            self.worker.obs_m_ph: worker_obs_m,
            self.worker.action_m_ph: worker_act_m
        }

        # Perform the update operations and collect the critic loss.
        message_loss, *_vals = self.sess.run(step_ops, feed_dict=feed_dict)
